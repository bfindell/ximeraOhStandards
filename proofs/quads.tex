...
\fixnote{The following details are subtle, especially distinguishing between mapping the sides (i.e., segments) and the lines containing the sides.  Can any of this be omitted or abbreviated?  Which parts might students supply?}
\begin{enumerate}
...
Because $B$ and $D$ are each equidistant from $A$ and $C$, they each must lie on the perpendicular bisector of $\overline{AC}$, which implies that 
...
\fixnote{Here, for the record, are the steps for a paragraph proof.
\begin{enumerate}
